...
representation. In this work, we presented a method for learning
...
if the environment changes over use cases requiring the user to specify
new action sequences to solve the same task, or if stochasticity in the
environment or actions can cause a specified action sequence to fail to complete
the task. Ideally, a human would specify
a task to complete with language and let the robot use a planner to be creative
in its solutions to the task and autonomously resolve any unforeseen
...
brining it to the user, without
...
can be easily transfered to different robots with different action sets,
since the agent will use a planner to determine the correct actions.

Our work is related to the planning approach described by \citet{howard14}. 
However, our work focuses
on grounding language to tasks best represented by Markov decision process (MDP)
reward functions
and planning under uncertainty, whereas \citet{howard14} focus on grounding language to
planning constraints for motion planning.

One way to learn groundings to reward functions would be to train a semantic
language model with a dataset consisting of pairs of natural language commands
and the corresponding reward function description they entail. However,
providing such a dataset requires a trainer to have a technical understanding
of the task and state representation of the robot. A more user-friendly approach
to training is for the user to develop a dataset of commands paired with
demonstrations of the command being carried out. This approach would also
...
and language; and second, a weakly supervised learning algorithm that uses
...
of the commands being carried out in the world.
...
We found that our task-grounding
...
These propositional functions enable an OO-MDP to provide high-level symbolic information about states that are inherently non-symbolic (e.g., spatial or continuous valued), which is often required for robotics domains. Because OO-MDP states are defined by a set of objects, different states in the same OO-MDP can also represent different environments by changing which objects are present. Since propositional functions operate on objects within a state, the propositional functions generalize across environments. In this work, we use OO-MDP propositional functions to define abstract task definitions, which enables environment-independent symbolic tasks useful for language grounding to be defined for a variety of different kinds of state spaces that may not be symbolic themselves.
...
\end{center}
\end{figure}
In this section we describe the overall execution and learning process of our approach. In the next section we will detail our generative model of tasks, behavior, and language that enables these processes. As a running example, consider the initial state of our {\em Cleanup World} domain shown in Figure~\ref{fig:ef}. Cleanup World (like Sokoban \cite{junghanns1997sokoban}) is a 2D grid world of various rooms connected by open doors. Rooms can also contain ``blocks'' that can be moved around. The robot moves using north-south-east-west actions. Moving into a location containing a block causes the block to move in the direction the agent is moving. If a wall or other item is in its immediate path, neither the agent nor the block moves. Cleanup World is represented as an OO-MDP consisting of four object classes: ${\tt ROBOT}$, ${\tt BLOCK}$, ${\tt ROOM}$, and ${\tt DOORWAY}$.
...
\begin{figure}
        ...
        \begin{subfigure}[b]{\columnwidth}
                ...
                \caption{A flow chart of command execution.}
                \label{fig:execute}
        \end{subfigure}%
        \\
        \begin{subfigure}[b]{\columnwidth}
                \includegraphics[width=\textwidth]{images/trainFlow}
                \caption{A flow chart of the training process.}
                \label{fig:learn}
        \end{subfigure}
        \caption{Flow charts for command execution and learning.}\label{fig:flow}
\end{figure}

Figure~\ref{fig:execute} shows a flow chart of the execution process. To illustrate the steps of this process, considering the command ``take the yellow chair to the red room'' being given in the state shown in Figure~\ref{fig:ef}. Using Bayesian inference with the learned model, the most likely task for the state shown in Figure\ref{fig:execute} is the goal condition ${\tt blockInRoom}(b_1, r_1)$, because the command uses words for taking an object to a location, and because the selected objects satisfy the properties ${\tt roomIsRed}(r_1) \land {\tt blockIsYellow}(b_1) \land {\tt blockIsChair}(b_1)$, which would have a high probability of generating the English words in the command. Once the most likely task has been inferred, a standard plan and control problem is instantiated, in which the robot uses a planning algorithm to generate a policy and follows the policy in a closed loop fashion.\footnote{The policy does not have to be complete and can invoke replanning if it is more efficient to do so.} Our approach is agnostic to the planning algorithm and can use any ``off-the-shelf'' planning algorithm appropriate for the domain, allowing our approach to scale to a large variety of robotics domains as long as enough of the state is observable to perform task grounding.

Figure~\ref{fig:learn} shows a flow chart of the learning process from taking a dataset of demonstrations paired with commands and learning the parameters of a language model that ground language to latent task descriptions. Let us return to the previous command ``take the yellow chair to the red room'' except this time in the role of a training instance paired with a demonstration started in the initial state shown in Figure~\ref{fig:ef} and following the action sequence $SSSEEEENNWNNESSS$, where $N, S, E, W$ stand for north, south, east, and west, respectively, resulting in the completion of the command's task. From this demonstration, inverse reinforcement learning (IRL) infers the probability that each possible task in the environment of the demonstration was the task being completed by the demonstration. In particular, the task with the goal ${\tt blockInRoom}(b_1, r_1)$ will have a very high probability compared to other tasks. This task has many possible descriptions based on the properties of the objects $b_1$ and $b_2$ to which language could be ground. In particular, one such task description is ${\tt blockInRoom}(b_1, r_1) {\tt roomIsRed}(r_1) \land {\tt blockIsYellow}(b_1) \land {\tt blockIsChair}(b_1)$. The set of all possible task descriptions for all tasks, weighted by the probability of their task being the intended one, form a set of weakly supervised labels for the training command and are used with a weakly supervised learning algorithm to train the language model parameters.

Our approach can easily use different semantic language models. In this work, we investigate two language models: a Bag-of-Words mixture model and
the IBM Model 2 used in statistical machine-translation~\cite{brown90,brown93} and use expectation maximization~\cite{dempster77} for training them with the weakly labeled commands. While we suspect that other language
models that incorporate grammatical knowledge might yield better
performance and robustness, an advantage of these grammar-free models
is that they do not require any additional corpus training and can be
...
There are a number of directions for future work including an investigation of grammar-based language models, more complex robotics tasks, more sophisticated task representations, grounding under partial observability, and commands that include subgoals. The grammar-free models have the advantage that they do not require a corpus for training the grammar model and they were sufficient for successfully grounding the language to tasks in our test domain. However, we imagine there are other domains in which more complex grammar-based language models could provide better results. 

In this work, our robotics task was fairly simply to facilitate gathering commands from an AMT study. However, because our approach is agnostic to the planning algorithm used in execution and learning, and because only the task description (rather than the state space) needs to be symbolic, we believe our approach can scale to a variety of more complex robotics tasks as long as enough of state is observable to perform task grounding. For example, we believe this work will scale to robotics domains like Forklift manipulation~\cite{teller2010voice}.

Currently, our tasks are described as a logical expression over a set of free variables. In the future, this could be extended to include first order logic quantifiers such as ``for all.'' Because tasks are represented with reward functions, our approach could also incorporate continuing task, rather than goal-directed tasks.
